...
3. [시스템 아키텍처 및 기술 스택 (Tech Stack)](#3-시스템-아키텍처-및-기술-스택-tech-stack)
4. [디자인 시스템 & UI/UX 가이드라인](#4-디자인-시스템--uiux-가이드라인)
5. [페이지별 화면 및 기능 명세](#5-페이지별-화면-및-기능-명세)
   - 5.1 메인 홈페이지 (`index.html`)
   - 5.2 커피 후원 플랫폼 (`donate/index.html`)
   - 5.3 전자상거래 3대 약관 및 정책 페이지
   - 5.4 개발 지침서 및 뷰어 (`donate/guide.html`)
6. [결제 인프라 및 SDK 연동 상세](#6-결제-인프라-및-sdk-연동-상세)
   - 6.1 국내 결제 (PortOne SDK V2)
   - 6.2 해외 결제 (PayPal Official Donate SDK)
7. [데이터베이스 스키마 및 보안 (Supabase)](#7-데이터베이스-스키마-및-보안-supabase)
8. [PG사/포트원 심사 5대 필수 요건 검증 결과](#8-pg사포트원-심사-5대-필수-요건-검증-결과)
9. [배포 및 운영 현황](#9-배포-및-운영-현황)
10. [향후 로드맵 및 고도화 계획](#10-향후-로드맵-및-고도화-계획)
...
* ⚖️ **전자상거래 100% 법적 준수**: 사업자 정보, 이용약관, 개인정보처리방침, 환불정책, 거래조건 고지 완비
...
## 4. 디자인 시스템 & UI/UX 가이드라인

### 4.1 메인 홈페이지: Vantablack Luxe & Glassmorphism
...
### 4.2 도네이션 플랫폼: Option A 웜 모카 & 크림 (Warm Mocha & Cream)
도네이션 페이지는 과도한 채도를 걷어내고, 눈의 피로를 덜어주는 아늑하고 신뢰감 있는 웜 톤을 기본 테마로 채택했습니다.
...
* 💸 **환불 및 청약철회 정책 (`donate/refund.html`)**: 결제 후 7일 이내 100% 청약철회/환불 보장, 3단계 환불 신청 절차 및 고객센터 유선번호(02-555-0123) 명시

---

### 5.4 개발 지침서 및 웹 뷰어 (`donate/guide.html`, `DONATION_GUIDE.html`)
* Paperlogy 웹폰트와 반응형 TOC(목차) 내비게이션, 상단 스크롤 진행 바, 다크모드 토글을 탑재한 인터랙티브 개발 가이드

---

## 6. 결제 인프라 및 SDK 연동 상세

### 6.1 국내 결제 (PortOne V2 Browser SDK)
```javascript
...
## 8. PG사/포트원 심사 5대 필수 요건 검증 결과

| 검증 항목 | 법적 요구사항 | 구축 및 반영 상태 |
| :--- | :--- | :---: |
| **1. 사업자 정보 유무** | 상호, 대표자, 사업자번호, 통신판매업, 주소, 전화번호, 이메일 | ✅ **100% 충족** |
| **2. 이용약관 유무** | 전자상거래 서비스 이용약관 독립 페이지 및 링크 연결 | ✅ **100% 충족** (`terms.html`) |
